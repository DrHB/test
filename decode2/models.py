...
__all__ = ['init_func', 'layer_types', 'extract_layer', 'FlexConvLayer', 'init_default', 'FlexUnetEncoder',
           'FlexUnetDecoder', 'SUNET', 'p_out_init', 'conv3d', 'create_conv', 'SingleConv', 'DoubleConv',
           'ExtResNetBlock', 'Encoder', 'Decoder', 'Upsampling', 'number_of_features_per_level', 'Abstract3DUNet',
           'UNet3D', 'ResidualUNet3D', 'OutNet']
...
# Cell
def init_default(m, func=nn.init.kaiming_normal_, bias=0.):
    "Initialize `m` weights with `func` and set `bias` to 0."
    if func and hasattr(m, 'weight'): func(m.weight)
    with torch.no_grad():
        if getattr(m, 'bias', None) is not None: m.bias.fill_(bias)
    return m
...
# Cell
p_out_init = partial(init_default, bias=-6.0, func = nn.init.constant_) #func = nn.init.constant_)
...
class UnetDecode(nn.Module):
    def __init__(self, ch_in: int =1, ch_out: int=10, final_sigmoid : bool =False, depth: int =3 ):
        super().__init__()
        self.bn = nn.BatchNorm3d(ch_in)
        self.unet=  UNet3D(ch_in, ch_out, final_sigmoid=final_sigmoid, num_levels=depth, layer_order ='bcr')
        
    def forward(self, x):
        norm_x = self.bn(x)
        out =  self.unet(norm_x)
        logit    = torch.clamp(out[:, 0], -8., 8)
        xyzi_sig = torch.sigmoid(out[:, 1:5]) + 0.01
        xyz_mu   = torch.tanh(out[:, 5:-2])
        i_mu     = torch.sigmoid(out[:, -2])
        background = F.softplus(out[:, -1])
...
class UnetDecodeInference(nn.Module):
    def __init__(self, ch_in: int =1, ch_out: int=10, final_sigmoid : bool =False, depth: int =3,  output='logits'):
        super().__init__()
        self.bn = nn.BatchNorm3d(ch_in)
        self.unet=  UNet3D(ch_in, ch_out, final_sigmoid=final_sigmoid, num_levels=depth, layer_order ='bcr')
        self.output = output 
    ...
    def forward(self, x):
        ...
        if self.output == 'xyzi_mu':
            return xyzi_mu
        if self.output == 'xyzi_sigma':
            return xyzi_sig 
        if self.output == 'background':
            return background.unsqueeze(1)
